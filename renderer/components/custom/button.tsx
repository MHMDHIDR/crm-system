...
export function Button({ children, className, withArrow, ...props }: ButtonProps) {
  return (
    <button
      className={cn(`relative h-10 font-medium bg-gradient-to-br group`, className)}
      {...props}
    >
      {props.variant !== 'link' ? (
        <span
...
function BottomGradient() {
  return (
    <>
      <span className='block absolute inset-x-0 -bottom-px w-full h-px bg-gradient-to-r from-transparent via-cyan-500 to-transparent opacity-0 transition duration-500 group-hover:opacity-100' />
      <span className='block absolute -bottom-px inset-x-10 mx-auto w-1/2 h-px bg-gradient-to-r from-transparent via-indigo-700 to-transparent opacity-0 blur-sm transition duration-500 group-hover:opacity-100' />
    </>
  )
}
